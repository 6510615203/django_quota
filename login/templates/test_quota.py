...
class QuotaTemplateTest(TestCase):

    def setUp(self):
        # สร้างข้อมูลทดสอบ เช่น ผู้ใช้งาน, นักเรียน, และวิชา
        ...
        # สร้างวิชาทดสอบ
        self.subject1 = Subject.objects.create(code='CN101', name='Introduction to Computer Programming', semester=1, year=2567, seats=50, status='AVAILABLE')
        self.subject2 = Subject.objects.create(code='CN102', name='Introduction to Computer Programming Lab', semester=1, year=2567, seats=0, status='FULL')

    def test_quota_template_renders_correctly(self):
        # ล็อกอินด้วยผู้ใช้ทดสอบ
        self.client.login(username='6510615096', password='usertest1')

        # เรียกหน้า quota
        response = self.client.get(reverse('login/templates/quota.html'))  # เปลี่ยน 'quota' เป็นชื่อ URL ที่ใช้กับ quota.html

        # ตรวจสอบว่า response status code เป็น 200
        self.assertEqual(response.status_code, 200)

        # ตรวจสอบว่าข้อมูลของผู้ใช้แสดงอยู่ในหน้า HTML
        self.assertContains(response, "ข้อมูลผู้ขอโควต้า : 6510615096 Nutpupicha Arungornpasuruk ชั้นปีที่ 3")

    def test_quota_template_displays_subject_list(self):
        # ล็อกอินด้วยผู้ใช้ทดสอบ
        self.client.login(username='6510615096', password='usertest1')
    ...
    def test_quota_template_displays_enroll_button_for_available_subject(self):
        # ล็อกอินด้วยผู้ใช้ทดสอบ
        self.client.login(username='6510615096', password='usertest1')
        ...
        # ตรวจสอบว่าปุ่ม Enroll ปรากฏในวิชาที่สถานะเป็น "AVAILABLE"
        self.assertContains(response, 'button type="submit" class="btn btn-primary">Enroll</button>')

    def test_quota_template_has_css_and_title(self):
        # ล็อกอินด้วยผู้ใช้ทดสอบ
        self.client.login(username='6510615096', password='usertest1')
        ...
        self.assertContains(response, "/static/css/quotastyle.css")
        self.assertContains(response, "<title>ขอโควต้า</title>")
